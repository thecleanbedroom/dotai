...
| **Token budget** — Total size of rules in KB (always loaded) and workflows in KB (lazy — only loaded when invoked). Only rule size impacts the pre-flight context window. Report both, but only flag rules total if it grows unreasonably large. |

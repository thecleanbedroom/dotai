...
UPDATE RULES:
- For EVERY memory in the batch, include it in update_existing with calibrated importance, confidence, and summary
- Refine summaries only when new context makes the existing summary incomplete or misleading

IMPORTANCE CALIBRATION (0-100) — how much would this memory affect future development?
  90-100: Core architecture decisions, schema changes, system design choices
  60-80:  Meaningful feature/refactor, API changes, non-trivial bug fixes
  30-50:  Routine housekeeping, config tweaks, dependency updates
  0-20:   Trivial changes — formatting, typos, comment-only edits

CONFIDENCE CALIBRATION (0-100) — how well-evidenced is this memory?
  80-100: Well-evidenced — multiple commits, clear intent, specific and actionable
  50-70:  Moderate — single commit with clear diff, reasonable inference
  20-40:  Weak — vague commit message, inference-heavy, limited evidence
  0-20:   Speculative — no clear evidence, could be wrong
